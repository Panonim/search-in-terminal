...
Release archives are named `sit_<version>_<os>_<arch>.tar.gz` (`.zip` on Windows) and each one contains
the `sit` binary plus `README.md` and `LICENSE`. A `checksums.txt` is attached to every release.

Pick your version and platform:

```sh
VERSION=1.0.0          # the release tag without the leading "v"
OS=linux               # linux | darwin | freebsd
ARCH=amd64             # amd64 | arm64

curl -fsSL -o sit.tar.gz \
  "https://github.com/Panonim/search-in-terminal/releases/download/v${VERSION}/sit_${VERSION}_${OS}_${ARCH}.tar.gz"
tar -xzf sit.tar.gz sit
install -m 0755 sit ~/.local/bin/sit
```

macOS (Apple silicon) in one line:

```sh
curl -fsSL "https://github.com/Panonim/search-in-terminal/releases/download/v1.0.0/sit_1.0.0_darwin_arm64.tar.gz" | tar -xz sit
```

Windows (PowerShell):

```powershell
Invoke-WebRequest -Uri "https://github.com/Panonim/search-in-terminal/releases/download/v1.0.0/sit_1.0.0_windows_amd64.zip" -OutFile sit.zip
Expand-Archive sit.zip -DestinationPath .
```

Verify a download against the release checksums:

```sh
curl -fsSLO "https://github.com/Panonim/search-in-terminal/releases/download/v1.0.0/checksums.txt"
...
```sh
sit update --check   # report the latest release, change nothing
sit update           # download, verify the checksum and replace the running binary
```

`sit update` needs write access to the directory holding the binary. If you installed into a
root-owned path, re-run it with `sudo` or reinstall from the archive.
